...
headers = {"x-api-key": API_HEADER_AUTHORIZATION}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user and not user.is_admin:
                ...
                return redirect(
                    "cliente_dashboard"
                )  # Asegúrate de tener esta vista y URL
            else:
...
def create_event_or_client(request, client_info, event_info, headers, api_url):
    ...
    response = requests.get(
        f"{api_url}clients/query/{client_info['email']}/", headers=headers
    )
    if response.status_code == 200:
        client_data = response.json()
        client_id = client_data["client_id"]
        update_response = requests.put(
            f"{api_url}clients/{client_id}/", headers=headers, json=client_info
        )
        if update_response.status_code != 200:
            print("Error al actualizar la información del cliente")
            print(
                update_response.json()
            )  # Esto imprimirá la respuesta completa para que puedas ver qué salió mal
        ...
        response = requests.post(
            f"{api_url}clients/create/", headers=headers, json=client_info
        )
        if response.status_code == 201:
            ...
            response = requests.post(
                f"{api_url}events/create/", headers=headers, json=event_info
            )
            return (
                client_id,
                "Evento creado exitosamente"
                if response.status_code == 201 or response.status_code == 200
                else "Error al crear el evento",
            )
    event_info["client_id"] = client_id
    response = requests.post(
        f"{api_url}events/create/", headers=headers, json=event_info
...
def fetch_client_by_email(request):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse(
            {"error": "Cliente no encontrado"}, status=response.status_code
        )
    ...
    citas = [
        {
            ...
            "hora": localtime(cita.scheduled_time).strftime("%H:%M - ")
            + localtime(cita.scheduled_time + timedelta(hours=1)).strftime("%H:%M"),
...
def book_appointment(request):
    ...
    scheduled_time = timezone.make_aware(
        scheduled_time_naive, timezone.get_current_timezone()
    )
    response = None
    ...
    if client_response.status_code == 200:
        ...
        event_api_url = (
            f"https://lbvj22e1he.execute-api.us-east-1.amazonaws.com/dev/events/create/"
        )
        headers = {
            "Content-Type": "application/json",
            "x-api-key": "IUPDlxEc2i2xxCYpmmnGL2JmqhVRkQba1n9Tbl6B",
...
def is_time_slot_available(user, start_time):
    ...
    overlapping_appointments = Appointment.objects.filter(
        scheduled_time__lt=end_time, end_time__gt=start_time
    )
...
def cliente_dashboard_documentos(request):
    documentos_usuario = Documento.objects.filter(usuario=request.user).order_by(
        "-fecha_actualizacion"
    )
    ...
    documentos_subidos = {
        tipo: True for tipo, doc in documentos_unicos.items() if doc.estado == "SUBIDO"
    }
    documentos_rechazados = {
        tipo: True
        for tipo, doc in documentos_unicos.items()
...
def upload_file(request):
    if request.method == "POST":
        ...
        client_response = fetch_client_by_email(request)
        if client_response.status_code == 200:
            client_data = client_response.content.decode(
                "utf-8"
            )  # Decode the JSON content
            client_data = json.loads(
                client_data
            )  # Parse the JSON content to get the dictionary
            ...
            if response.status_code == 200 or response.status_code == 201:
                ...
                return JsonResponse(
                    {"status": "Error", "message": "Error al crear el evento."},
...
def notification_settings(request):
    ...
    if request.method == "POST":
        ...
        settings.receive_notifications = (
            request.POST.get("receive_notifications") == "on"
        )
        settings.notify_document_status = (
            request.POST.get("notify_document_status") == "on"
        )
        settings.notify_appointment_confirmation = (
            request.POST.get("notify_appointment_confirmation") == "on"
        )
        settings.notify_appointment_reminder = (
            request.POST.get("notify_appointment_reminder") == "on"
        )
        settings.save()
    ...
    current_settings = UserNotificationSettings.objects.get_or_create(
        user=request.user
    )[0]
    return render(
        request,
        "notification_settings.html",
        {"settings": current_settings, "notificaciones": notificaciones},
    )
...
def notifications(request):
    ...
    notificaciones_list = Notificacion.objects.filter(usuario=request.user).order_by(
        "-fecha_creacion"
    )
...
def eliminar_notificacion(request, notificacion_id):
    if (
        request.method == "POST"
    ):  # Asegúrate de que este argumento coincida con el nombre en urls.py
        notificacion = get_object_or_404(
            Notificacion, id=notificacion_id, usuario=request.user
        )
        notificacion.delete()
        return redirect(
            "notifications"
        )  # Reemplaza esto con el nombre real de tu vista
...
def marcar_leida_notificacion(request, id_notificacion):
    try:
        notificacion = Notificacion.objects.get(
            id=id_notificacion, usuario=request.user
        )
